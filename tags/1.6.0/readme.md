# SOCICON WordPress Plugin

[demo](https://fatpony.me/plugins/socicon/)

The easiest way to add the social icon font [Socicon](https://socicon.teddypayet.com/) to your WordPress site! Instant access to over 300 social icons, Socicon is a *lightweight* alternative to FontAwesome for displaying social icons.

Socicon's original maintainers are no longer active. This plugin now bundles the font from the actively-maintained community fork at [github.com/Ybbet/socicon](https://github.com/Ybbet/socicon).

![Socicon is a lightweight social icon font, now available as a WordPress plugin.](https://fatpony.me/wp-content/uploads/2015/08/banner-772x250.jpg)

### Icons

The following icons are available with Socicon:  

1. 500px
2. 8tracks
3. Adobe Portfolio
4. Airbnb
5. Alibaba
6. Aliexpress
7. Alliance
8. Amazon
9. amplement
10. Android
11. AngelList
12. Angie's List
13. App Store
14. Apple
15. appnet
16. ArtStation
17. Ask
18. AUGMENT
19. Avvo
20. baidu
21. Bale
22. bandcamp
23. BATTLE.NET
24. Beatport
25. beBee
26. bebo
27. beehiiv
28. Behance
29. BeReal
30. besport
31. Bing
32. Bitbucket
33. BlackBerry
34. Blizzard
35. Blogger
36. BLOGLOVIN'
37. bluesky
38. Bonanza
39. BookBub
40. Booking
41. BookWyrm
42. Buffer
43. Buy Me a Coffee
44. Calendly
45. CD Baby
46. Chrome
47. Codeberg
48. Codepen
49. CodeRED
50. coderwall
51. Collectorz
52. Craigslist
53. crunchbase
54. Curse
55. Dailymotion
56. debian
57. DEEZER
58. Delicious
59. deviantART
60. Diablo
61. Digg
62. Discord
63. DISQUS
64. DLive
65. Doodle
66. douban
67. draugiem.lv
68. dribbble
69. Drupal
70. ebay
71. Eitaa
72. Element
73. elementary OS
74. Ello
75. endomondo
76. Envato
77. Etsy
78. Facebook
79. Facebook Messenger
80. FeedBurner
81. FilmFreeway
82. FILMWEB
83. Firefox
84. Fiverr
85. Flattr
86. flickr
87. Flipboard
88. Forgejo
89. FORMULR
90. Forrst
91. foursquare
92. Freelancer
93. FriendFeed
94. Fundable
95. Funkwhale
96. FYUSE
97. Game Jolt
98. GameFor
99. GameWisp
100. ghost
101. GitHub
102. GitLab
103. GITTER
104. GNOME
105. goodreads
106. Google
107. Google Calendar
108. Google Groups
109. Google Hangouts
110. Google Maps
111. Google Photos
112. Google Play
113. Google Play Music
114. Google Scholar
115. Google+
116. GoToMeeting
117. grooveshark
118. Guru
119. Gust
120. Hacker News
121. hackerone
122. HackerRank
123. Hearthstone
124. Hellocoton
125. Hereos of the Storm
126. hitbox
127. HomeAdvisor
128. Homes
129. homify
130. Horde
131. houzz
132. icq
133. Identica
134. IMDb
135. indie DB
136. Instagram
137. instructables
138. Internet
139. InVision
140. issuu
141. iStock
142. itch.io
143. iTunes
144. JAMENDO
145. Keybase
146. Kick
147. Ko-fi
148. Kobo
149. Lanyrd
150. last.fm
151. Lemmy
152. Letterboxd
153. LINE
154. Linkedin
155. Linktree
156. LiveJournal
157. livemaster
158. LogMeIn
159. loomly
160. lyft
161. macOS
162. Mail
163. Mail.ru
164. Mastodon
165. Matrix
166. Mbin
167. Medium
168. Meetup
169. Metapop
170. Microsoft
171. Misskey
172. MIX
173. Mixcloud
174. Mixer
175. mobcrush
176. Mobilizon
177. Mod DB
178. Model Mayhem
179. Mozilla Persona
180. mumble
181. MyAnimeList
182. Myspace
183. napster
184. Natgeo
185. NAVER
186. Nebula
187. NewsVine
188. Nextdoor
189. niconico
190. Nintendo Network
191. npm
192. Odnoklassniki
193. OpenAI Gym
194. OpenID
195. Opera
196. Origin
197. Outlook
198. Overwatch
199. Pandora
200. Patreon
201. Paypal
202. PeerTube
203. Periscope
204. Pinterest
205. Pixelfed
206. pixiv
207. Player.me
208. PlayStation
209. Pocket
210. Qobuz
211. QQ
212. Quora
213. RaidCall
214. Ravelry
215. Realtor
216. REDBUBBLE
217. reddit
218. Redfin
219. remote
220. renren
221. ResearchGate
222. Resident Advisor
223. REVERBNATION
224. RSS
225. Rumble
226. Seedrs
227. SeenThis
228. SharePoint
229. ShareThis
230. Shopify
231. Signal
232. Sina Weibo
233. Sketchfab
234. skype
235. Slack
236. SlideShare
237. smashcast
238. Smashwords
239. SmugMug
240. Snapchat
241. society6
242. songkick
243. Soroush
244. soundcloud
245. SPIP
246. Splice
247. Spotify
248. spreadshirt
249. Squarespace
250. StackExchange
251. stackoverflow
252. STAGE32
253. Starcraft
254. StayFriends
255. steam
256. Stitcher
257. Storehouse
258. STRAVA
259. StreamJar
260. StumbleUpon
261. Substack
262. Swarm
263. TeamSpeak
264. TeamViewer
265. Technorati
266. Telegram
267. threads
268. Threema
269. Tidal
270. TikTok
271. Tinder
272. ToneDen
273. Toptal
274. Torial
275. Traxsource
276. Trello
277. tripadvisor
278. Tripit
279. triplej
280. Trulia
281. tumblr
282. tunein
283. Twitch
284. Twitter
285. UBER
286. Ubuntu
287. udemy
288. Unsplash
289. Upwork
290. Ventrilo
291. Viadeo
292. Viber
293. viewbug
294. vimeo
295. Vine
296. VKontakte
297. VSCO
298. Warcraft
299. WeChat
300. WhatsApp
301. Wickr
302. Wikipedia
303. Windguru
304. Windows
305. Wix
306. WordPress
307. wykop
308. x
309. XBOX
310. Xing
311. Yahoo!
312. Yammer
313. Yandex
314. yelp
315. Younow
316. YouTube
317. Youtube Gaming
318. Zapier
319. Zazzle
320. Zerply
321. Zillow
322. zomato
323. Zoom
324. zynga

## Installation

1. Install Socicon by uploading the files to the /wp-content/plugins/ directory via FTP.
2. Activate Socicon.
3. Begin adding Socicon icons throughout your site!

## Usage

See the [Socicon Documentation](https://fatpony.me/plugins/socicon/) for instructions on using Socicon on your WordPress site.

## Support

Please [open an issue](https://github.com/ericakfranz/socicon/issues/new) for support.

## Contributing

Please contribute using [Github Flow](https://guides.github.com/introduction/flow/). Create a branch, add commits, and [open a pull request](https://github.com/ericakfranz/socicon/compare/).

## Credits

- Font, [Socicon](https://socicon.teddypayet.com/) is released under the [SIL Open Font License 1.1](http://scripts.sil.org/OFL). Bundled from the community-maintained fork at [github.com/Ybbet/socicon](https://github.com/Ybbet/socicon).

## Changelog

- v1.6.0
    - Version bump, tested up to WordPress 7.1.0
    - Switched the bundled Socicon font to the actively-maintained community fork at github.com/Ybbet/socicon (v3.10.1), since the original socicon.com maintainers are no longer active (see GitHub issue #12)
    - Icon count increased from 167 to 324; see the full list above
    - Renamed icon classes: `socicon-beam` is now `socicon-mixer`, and `socicon-endomodo` (a typo in the old font) is now the correctly-spelled `socicon-endomondo` — update any custom CSS or templates that reference these two classes directly
    - Added a `socicon-x` class for the X (formerly Twitter) rebrand; the existing `socicon-twitter` class is unchanged and still works
    - Dropped the bundled .woff2 font file, which the new upstream font no longer ships; the plugin now serves .eot/.woff/.ttf/.svg, which covers all supported browsers

- v1.5.1
    - Version bump, compatible with WordPress 6.0.1

- v1.5.0
    - Compatible with WordPress 4.8.1
    - Added Icons: BeBee, Hitbox, ReverbNation, Formulr, Discord, Issuu, Firefox, Chrome, Opera, Battle.net, Warcraft, Diablo, Hearthstone, Heroes of the Storm, Overwatch, StarCraft, Keybase, LiveJournal, Etsy, Google Photos, Google Scholar, Zapier, ResearchGate, WeChat, Strava, LINE, Lyft, Uber, Songkick, ViewBug, Google Groups, Blizzard, HackerRank, npm

- v1.4.0
    - Compatible with WordPress 4.5
    - Add Icons: Medium, Telegram, OpenID, Amplement, Viber, Tomato, Quora, Dragiem, Endomondo, Filmweb, Stackexchange, Wykop, Teamspeak, Ventrilo, Raidcall, Mumble, Younow and Teamviewer.
    - Update Socicon font files

- v1.3.3
    - Fix PHP Notice: Use of undefined constant screen - assumed 'screen' in /wp-content/plugins/socicon/socicon.php on line 17
- v1.3.1
    - Version bump, compatible with WordPress 4.4
    - Update plugin screenshots

- v1.3.0
    - Add Icons: Snapchat, Weibo, Douban, Baidu, QQ, Renren, Google, Yandex, eBay, IMDb, StayFriends, DEEZER, ShareThis, band camp, iTunes and Resident Advisor.
    - Remove Icon: Chime.in
    - Update Socicon font files
    - Update organization of icons in stylesheet for easier maintenance.
    - Add support for class ‘socicon-google-play’ to display Play Store icon.

- v1.1.0
    - Add Icon: Ravelry
    - Update Icons: Google+, YouTube
    - Update CSS for better font-rendering
- v1.0.2 FIX: Chime.in icon CSS
- v1.0.1 Add Icons: WhatsApp, Storehouse, icq, Model Mayhem, iStock, AngelList and Periscope.
- v1.0.0 Version bump
    - Add Gulp and Sass  
    - Make CSS selectors more specific to prevent conflicts
